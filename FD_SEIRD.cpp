...
int FD()
{
    ...
    for(int y=0; y<ny; y++)
      ...
      N = rhoS_old[id]+rhoE_old[id]+rhoI_old[id]+rhoR_old[id]+rhoD_old[id];
			rhoS[id] = rhoS_old[id] + 0.5*Scale_time*(-phys_beta*rhoS_old[id]*rhoI_old[id]/N + phys_dS*laplS/pow(Scale_length,2));
			rhoE[id] = rhoE_old[id] + 0.5*Scale_time*(phys_beta*rhoS_old[id]*rhoI_old[id]/N - phys_alpha*rhoE_old[id] + phys_dE*laplE/pow(Scale_length,2));
			rhoI[id] = rhoI_old[id] + 0.5*Scale_time*(phys_alpha*rhoE_old[id] - phys_gamma*rhoI_old[id] + phys_dI*laplI/pow(Scale_length,2));
			rhoR[id] = rhoR_old[id] + 0.5*Scale_time*(phys_gamma*(1.-phys_phi)*rhoI_old[id] + phys_dR*laplR/pow(Scale_length,2));
			rhoD[id] = rhoD_old[id] + 0.5*Scale_time*phys_gamma*phys_phi*rhoI_old[id];
		}
	for(int x=0; x<nx; x++)
    for(int y=0; y<ny; y++)
		{
	    ...
	    N = rhoS[id]+rhoE[id]+rhoI[id]+rhoR[id]+rhoD[id];
			rhoS_old[id] = rhoS[id] + 0.5*Scale_time*(-phys_beta*rhoS[id]*rhoI[id]/N + phys_dS*laplS/pow(Scale_length,2));
			rhoE_old[id] = rhoE[id] + 0.5*Scale_time*(phys_beta*rhoS[id]*rhoI[id]/N - phys_alpha*rhoE[id] + phys_dE*laplE/pow(Scale_length,2));
			rhoI_old[id] = rhoI[id] + 0.5*Scale_time*(phys_alpha*rhoE[id] - phys_gamma*rhoI[id] + phys_dI*laplI/pow(Scale_length,2));
			rhoR_old[id] = rhoR[id] + 0.5*Scale_time*(phys_gamma*(1.-phys_phi)*rhoI[id] + phys_dR*laplR/pow(Scale_length,2));
			rhoD_old[id] = rhoD[id] + 0.5*Scale_time*phys_gamma*phys_phi*rhoI[id];
